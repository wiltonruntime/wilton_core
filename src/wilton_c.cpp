...
char* wilton_Server_create(
    ...
    if (conf_json_len <= 0 ||
            conf_json_len > std::numeric_limits<uint32_t>::max()) return su::alloc_copy(TRACEMSG(std::string() +
...
char* wilton_Request_get_request_metadata(wilton_Request* request, const char** metadata_json_out,
    ...
    if (nullptr == metadata_json_out_len) return su::alloc_copy(TRACEMSG(std::string() +
            "Null 'metadata_json_len_out' parameter specified"));
    try {
        ss::JsonValue json = request->impl().get_request_metadata();
        std::string res = ss::dump_json_to_string();
...
const char* wilton_Request_get_request_data(wilton_Request* request, const char** data_out,
    ...
    try {
        std::string& res = request->impl().get_request_data();
        *data_out = res.c_str();
...
char* wilton_Request_set_response_metadata(wilton_Request* request,
    ...
    if (metadata_json_len <= 0 ||
            metadata_json_len > std::numeric_limits<uint32_t>::max()) return su::alloc_copy(TRACEMSG(std::string() +
...
char* wilton_Request_send_response(wilton_Request* request, const char* data,
    ...
    if (data_len <= 0 ||
            data_len > std::numeric_limits<uint32_t>::max()) return su::alloc_copy(TRACEMSG(std::string() +
