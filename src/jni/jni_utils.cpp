...
jclass find_java_class(JNIEnv* env, const std::string& name) {
    ...
    if (nullptr == local) {
        throw common::WiltonInternalException(TRACEMSG("Cannot load class, name: [" + name + "]"));
    }
    jclass global = static_cast<jclass>(env->NewGlobalRef(local));
    if (nullptr == global) {
        throw common::WiltonInternalException(TRACEMSG("Cannot create global ref for class, name: [" + name + "]"));
    }
